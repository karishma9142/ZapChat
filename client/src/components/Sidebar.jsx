...
function Sideber() {
    const { getUsers, users, selectedUser, setSelectedUser ,setUnseenMessage , unseenMessage} = useContext(ChatContext);
    ...
    return (
        ...
        text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
            <div className="pb-5">
                ...
                <div className="flex items-center gap-3 bg-[#282142] py-3 px-4 mt-5 rounded-full">
                    <img src={assets.search_icon} alt="search" className="w-4" />
                    <input onChange={(e)=>setInput(e.target.value)} type="text" placeholder="Search User..." className="bg-transparent border-none outline-none
                    text-white text-sm placeholder-[#c8c8c8] flex-1"/>
                </div>
            </div>
                ...
                {filteredUsers.map((user, index) => (
                    ...
                    >
                        ...
                        {unseenMessage[user._id] >0 && (
                            <p className="absolute top-4 right-4 text-xs h-5 w-5 flex
